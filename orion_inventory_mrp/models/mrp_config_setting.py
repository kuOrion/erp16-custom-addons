...
class ResConfigSettings(models.TransientModel):
    ...
    td_manufactured_location_id = fields.Many2one(
        ...
        config_parameter='orion_inventory_mrp.td_manufactured_location_id')

    td_fg_location_id = fields.Many2one(
        'stock.location', string='FG Location',
        domain=[('usage', '=', 'internal')],
        config_parameter='orion_inventory_mrp.td_fg_location_id')

    td_stock_location_id = fields.Many2one(
        'stock.location', string='Stock Location',
        domain=[('usage', '=', 'internal')],
        config_parameter='orion_inventory_mrp.td_stock_location_id')
    ...
    def get_default_route_location(self, fields):
        ...
        td_stock_location_id = IrConfigParam.get_param('orion_inventory_mrp.td_stock_location_id')
        return {
    ...
    def set_route_location(self):
        self.ensure_one()
        IrConfigParam = self.env['ir.config_parameter']
        IrConfigParam.set_param('orion_inventory_mrp.td_manufactured_location_id', self.td_manufactured_location_id.id or '')
        IrConfigParam.set_param('orion_inventory_mrp.td_fg_location_id', self.td_fg_location_id.id or '')
        IrConfigParam.set_param('orion_inventory_mrp.td_stock_location_id', self.td_stock_location_id.id or '')
