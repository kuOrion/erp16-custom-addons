...
class SaleOrder(models.Model):
    ...
    current_amendment_id = fields.Many2one('sale.order', 'Current Amendment', readonly=True, copy=True)
    old_amendment_ids = fields.One2many('sale.order', 'current_amendment_id', 'Old Amendment', readonly=True,
    ...
    def copy(self, defaults=None):
        ...
        if self.env.context.get('new_sale_amendment'):
            ...
            revno = self.amendment_no
            defaults.update({
                'name': f"{self.amendment_name}-AMD-{revno + 1:02d}",
                'amendment_no': revno + 1,
                'state': 'cancel',
                'invoice_count': 0,
    ...
    def go_amendment(self):
        for sale in self:
            for pick in sale.picking_ids:
                if pick.state == 'done':
                    raise UserError(
                        'Unable to amend this sales order. You must first cancel all receptions related to this sales order.')
                pick.filtered(lambda r: r.state != 'cancel').action_cancel()

            for inv in sale.invoice_ids:
                if inv.state in ['done', 'open']:
                    raise UserError(
                        'Unable to amend this sales order. You must first cancel all Customer Invoices related to this sales order.')
                inv.filtered(lambda r: r.state != 'cancel').action_invoice_cancel()
